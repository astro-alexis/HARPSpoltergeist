...
h1 = fits.open(f[0])[0].header
h2 = fits.open(f[1])[0].header
h3 = fits.open(f[2])[0].header
h4 = fits.open(f[3])[0].header
...
wf = glob.glob("1-beam-wl-solution.npz")[0]
thard = load(glob.glob("1-beam-wl-solution.npz")[0])
wave = thard['wave']
...
for i in range(int(f1['SPEC'].shape[1]/2)):
    ...
    wd,wu = wave[i,:],wave[i,:]
    data = {'WAVE' : np.empty(len(wd))}
    data['WAVE'] = wd
    # Load blaze
    data['BLAZE_DOWN'] = blaze[2*i,:]
    mblad = np.nanpercentile(data['BLAZE_DOWN'],50)
    splblad = UnivariateSpline(data['WAVE'][15:-15],data['BLAZE_DOWN'][15:-15]/mblad, s=.05)

#    splblaup = UnivariateSpline(data['WAVE'][15:-15],data['BLAZE_UP'][15:-15]/mblaup, s=.05)
    cblad = splblad(data['WAVE'])
#    cblaup = splblaup(data['WAVE'])
    ...
    d1,d2,d3,d4 = d1/cblad,d2/cblad,d3/cblad,d4/cblad, 
    ed1,ed2,ed3,ed4 = ed1/cblad,ed2/cblad,ed3/cblad,ed4/cblad
    u1,u2,u3,u4 = u1/cblad,u2/cblad,u3/cblad,u4/cblad,  
    eu1,eu2,eu3,eu4 = eu1/cblad,eu2/cblad,eu3/cblad,eu4/cblad
    # Compute mean up and down spectra
    meanu,meand = np.nanmedian([u1,u2,u3,u4],axis=0), np.nanmedian([d1,d2,d3,d4],axis=0)
    ...
    coefd,cId,fmd,idxd = FitCon(data['WAVE'],meand,deg=2,niter=150,sig=meaned,swin=500,k1=.1,k2=.3,mask=mask)

    data['CONTINUUM'] = cId * 8.

    # Compute ratio for demodulation
    R = u1i/d1 * u3i/d3 / (u2i/d2 * u4i/d4)
    ...
    # Compute NULL
    RN = u1i/d1 * u2i/d2 / (u3i/d3 *u4i/d4) 
    ...
    if plot == 'yes':
        ...
        ax[0].plot(data['WAVE'][15:-15],data['INTENS_NORM'][15:-15], "k", linewidth=1)
        ax[0].plot(data['WAVE'][idxd],data['INTENS_NORM'][idxd], marker=".", ms=1, linewidth=0)
        ...
        ax[2].plot(data['WAVE'][15:-15],np.zeros(len(data['STOKES/Ic'][15:-15])), color="r", linestyle="dashed", linewidth=0.5)
        ax[2].set_ylabel('Stokes $V/Ic$')
...
starname = h1['OBJECT'].replace('-',' ').replace('_',' ')
half_exptime =  (h1['MJD-OBS'] + h4['MJD-OBS'] + (h4['EXPTIME'] / 86400.) ) / 2.
#145     # Convert half_exptime from seconds to day
jd = Time(2400000.5 + half_exptime, format='jd', scale='utc')
#147      # Convert JD_UTC to UTC_TDB
utc_tbd = utc_tdb.JDUTC_to_BJDTDB(jd, starname=starname,lat=h1['HIERARCH ESO TEL GEOLAT'],
          longi=h1['HIERARCH ESO TEL GEOLON'], alt=h1['HIERARCH ESO TEL GEOELEV'])
bjd_tdb = np.squeeze(utc_tbd[0])

#152     # Compute BERV
bcvel = get_BC_vel(JDUTC=jd, starname=starname, lat=h1['HIERARCH ESO TEL GEOLAT'], 
                                 longi=h1['HIERARCH ESO TEL GEOLON'], alt=h1['HIERARCH ESO TEL GEOELEV'], zmeas=0.0)
#155     data['berv'][i] = np.squeeze(bcvel[0])
print("BERV: ", bcvel[0])
...
#hdul_output[0].header['BERV'] =  (bcvel, 'barycentric velocity from barycorrpy in m/s' )
hdul_output.writeto('HARPSpol_demodulated.fits', overwrite=True)
